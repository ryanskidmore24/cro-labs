...
const FROM = process.env.EMAIL_FROM || 'Conversion Path <noreply@conversionpath.com>';
...
export async function sendVerificationEmail(to: string, name: string, token: string) {
  ...
  await getResend().emails.send({
    from: FROM,
    to,
    subject: 'Verify your Conversion Path email',
    html: emailLayout(`
      <h2 style="margin:0 0 16px">Welcome to Conversion Path${name ? `, ${name}` : ''}!</h2>
...
export async function sendPasswordResetEmail(to: string, token: string) {
  ...
  await getResend().emails.send({
    from: FROM,
    to,
    subject: 'Reset your Conversion Path password',
    html: emailLayout(`
...
export async function sendInviteEmail(to: string, inviterName: string, orgName: string, token: string) {
  ...
  await getResend().emails.send({
    from: FROM,
    to,
    subject: `${inviterName} invited you to ${orgName} on Conversion Path`,
    html: emailLayout(`
      <h2 style="margin:0 0 16px">You're invited!</h2>
      <p style="margin:0 0 8px;color:#6b7280"><strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on Conversion Path.</p>
      <p style="margin:0 0 24px;color:#6b7280">Conversion Path is an AI-powered conversion rate optimization platform.</p>
      ...
      <table width="100%" style="max-width:520px;background:#fff;border-radius:12px;border:1px solid #e5e7eb;padding:40px">
        <tr><td>
          <div style="margin-bottom:32px">
            ...
            <span style="margin-left:10px;font-size:18px;font-weight:700;color:#111827;vertical-align:middle">Conversion Path</span>
          </div>
          ${body}
          <hr style="border:none;border-top:1px solid #f3f4f6;margin:32px 0">
          <p style="margin:0;color:#9ca3af;font-size:12px">Conversion Path · AI-powered conversion rate optimization</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
